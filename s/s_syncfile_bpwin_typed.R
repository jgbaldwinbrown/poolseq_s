...
slide = Data.frame() ? function(data= ? Data.frame(), chrcol= ? Character(1), bpcol= ? Character(1), winsize, winstep) {
    Data.frame() ? out
    out = data.frame(Chrom=character(), Start=numeric(), End=numeric())
    for (chrom in levels(factor(data[,chrcol]))) {
        Data.frame() ? chromdata

        max = max(data[,bpcol][data[,chrcol] == chrom]) + (winsize*2)
        start = seq(1,max,winstep)
        end = start + (winsize-1)
        chroms = rep(chrom, length(start))
        chromdata = data.frame(Chrom = chroms, Start = start, End = end)
        out = as.data.frame(rbind(out, chromdata))
    }
    return(out)
}

slide_data = List() ? function(wins= ? Data.frame(), data= ? Data.frame(), chrcol= ? Character(1), bpcol= ? Character(1)) {
    get_data = function(x= ? Character()) {
        return(data[data[,chrcol] == as.character(x["Chrom"]) && data[,bpcol] >= as.numeric(x["Start"]) && data[,bpcol] <= as.numeric(x["End"]),])
    }
    return(apply(wins, 1, get_data))
}
...
#     print(str(chrom_pos_list))
#     print(chrom_pos_list)
#     chrom_pos = as.data.frame(do.call("rbind", chrom_pos_list))
#     return(list(chrom_pos, s_vals_list))
# }

combine_all_chunks = List(2) ? function(wins= ? Data.frame(), s_outputs= ? List()) {
    List() ? s_vals_list()
    # s_vals_list = unlist(mclapply(full_output_list, function(x) x[[2]]), recursive=FALSE)
    s_vals_list = unlist(s_outputs, recursive=FALSE)
    return(list(wins, s_vals_list))
}

estimateSH_wins <- List() ? function(sync, Ne= ? Double(), info= ? List(), chrompos= ? Data.frame(), start_chrompos_chunk= ? List(), winsize= ? Integer()) {
    # Need to update this so start_chrompos_chunk, start_chrompos_chunk_list correctly use bp sets
    ...
    List() ? s_vals
    s_vals = unname(mclapply(start_chrompos_chunk, function(x) {estimateSH_one_win(sync, Ne, info, chrompos, x, winsize)}))
    # print("length(s_vals):")
    # print(length(s_vals))
    return(s_vals)
}
...
estimateSH_win_savewrapper <- List(2) ? function(sync, Ne= ? Double(), info= ? List(), outpath= ? Character(1), winsize= ? Integer(1), winstep= ? Integer(1)) {
    # Need to update this function to make all_starts, all_starts_chunked and starts_chunked use bp insead of fixed snp
    Data.frame() ? chrompos
    ...
    Data.frame() ? all_wins
    all_wins = slide(chrompos, "chrom", "pos", winsize, winstep)

    List() ? all_wins_data
    all_wins_data = slide_data(all_wins, chrompos, "chrom", "pos")
    if (length(all_wins_data) > 0) {
        all_wins_chunked = split(all_wins, (0:(length(all_wins)-1)) %/% global_chunksize)
        all_wins_data_chunked = split(all_wins_data, (0:(length(all_wins_data)-1)) %/% global_chunksize)
    } else {
        all_wins_chunked = list(all_wins)
        all_wins_data_chunked = list(all_wins_data)
    }
    ...
    # # print(all_starts_chunked)
    full_output_list = vector(mode = "list", length = length(all_wins_data_chunked))
    j = 1
    for (i in 1:length(all_wins_data_chunked)) {
	...
	Character(1) ? temppath_est_sh_done
        ...
        if (! file.exists(temppath_est_sh_done)) {
            List() ? start_chunk
            start_chunk = all_wins_data_chunked[[i]]
            # print("start_chunk:")
            # print(start_chunk)
            s_vals = estimateSH_wins(sync, Ne, info, chrompos, start_chunk, winsize)
            full_output_list[[j]] = s_vals
            saveRDS(s_vals, file = temppath_est_sh)
    ...
    List(2) ? out2
    print(str(full_output_list))
    
    out2 = combine_all_chunks(all_wins, full_output_list)
...
est_full_save_win <- Data.frame() ? function(sync, Ne= ? Double(), info= ? List(), outpath= ? Character(1), winsize= ? Integer(1), winstep= ? Integer(1)) {
    ...
    print("str(chrom_pos)")
    print(str(chrom_pos))
    chrom_pos$Mid = (chrom_pos$Start + chrom_pos$End) / 2
    out = as.data.frame(cbind(chrom_pos[,c("Chrom", "Mid")], s_vals, p_vals))
...
est_full_save_repls_win <- List() ? function(sync, Ne= ? Double(), info= ? List(), outpath= ? Character(1), winsize= ? Integer(1), winstep= ? Integer(1)) {
    ...
    for (i in 1:length(out)) {
        ...
        # print(p_vals)
        chrom_pos$Mid = (chrom_pos$Start + chrom_pos$End) / 2
        out[[i]] = as.data.frame(cbind(chrom_pos[,c("Chrom", "Mid")], s_vals, p_vals))
        # print(out)
